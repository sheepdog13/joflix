    ...
    movie && (
      <article>
        <div className={styles.overlay}>
          <motion.div layoutId={movie.id + ''} transition={{ type: false }} className={styles.wrapper}>
            <div
              className={styles.coverBox}
              style={{
                backgroundImage: `linear-gradient(rgba(0, 0, 0, 0), rgba(20, 20, 20, 1)), url(${makeImagePath(movie.backdrop_path)})`,
              }}
            >
              <Link
              ...
              <div className="mb-14 ml-5 sm:mb-24 sm:ml-0">
                ...
                <div className={styles.btnBox}>
                  <div className="flex items-center p-1.5 gap-0.5 rounded-md bg-white text-black font-medium hover:bg-white/80 sm:px-3 sm:py-2 sm:gap-1">
                    <SvgIcon className=" text-2xl sm:text-3xl" component={PlayArrowIcon} />
                    <button>재생</button>
                  </div>
                  <Button width="45" fontsize="30" component={AddIcon} />
                  <Button width="45" fontsize="24" component={ThumbUpOffAltIcon} />
                </div>
              </div>
            </div>
            <div className={styles.content}>
              <div className={styles.descBox}>
                <div className={styles.leftBox}>
                  ...
                  <div className="text-white line-clamp-6 sm:line-clamp-none">{movie.overview}</div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {similars &&
                    similars.slice(0, 9).map((similar) => (
                      <Link key={similar.id} href={{ query: { id: similar.id } }}>
