...
export default function Header() {
  ...
  return (
    <motion.header variants={navVariants} animate={navAnimation} className={styles.wrapper}>
      <div className="flex">
        <Link href={'/'}>
          <h1 className="font-Cart font-extrabold text-lg text-red-600  sm:text-4xl mr-8">JOFLIX</h1>
        </Link>
      ...
      <div className={styles.userActionsContainer}>
        ...
        >
          ...
          {searchOpen && (
            <motion.form onSubmit={handleSubmit(onValid)} animate={{ scaleX: searchOpen ? 1 : 0 }} transition={{ type: 'linear' }}>
              <input
                autoComplete="off"
        ...
        <div>
          ...
          <SvgIcon className="hidden" component={ArrowDropDownIcon} fontSize="small" />
        </div>
      </div>
    </motion.header>
  );
